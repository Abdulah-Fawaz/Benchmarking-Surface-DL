...
from meshcnn.ops import MeshConv, MeshConv_transpose, ResBlock_Mod
...
class Up(nn.Module):
    def __init__(self, in_ch, out_ch, level, mesh_folder, bias=True):
        ...
        self.conv = ResBlock_Mod(in_ch, out_ch, out_ch, level, False, mesh_folder)
...
class Down(nn.Module):
    def __init__(self, in_ch, out_ch, level, mesh_folder, bias=True):
        ...
        """
        super().__init__()
        self.conv = ResBlock_Mod(in_ch, in_ch, out_ch, level+1, True, mesh_folder)
...
class SphericalUNet(nn.Module):
    def __init__(self, mesh_folder, in_ch, out_ch, max_level=6, min_level=1, fdim=16):
        super().__init__()
